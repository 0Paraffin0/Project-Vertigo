...
// ─── STATIC DATA  (Phase 7 replaces with live AI feed) ────────────────────────
const MOCK_ARTICLES = [
  {
    id: 1,
    category: "breaking",
    ...
    brief:
      "The Federal Reserve kept its benchmark rate at 5.25–5.5%, citing easing inflation but cautioning that labour market data must soften further before cuts begin.",
  ...
  },
  {
    ...
    brief:
      "US regulators are examining whether Deloitte, PwC, KPMG and EY have engaged in anticompetitive practices in the audit market, potentially forcing structural separation.",
  ...
  },
  {
    ...
    brief:
      "London equities gained 0.4% after UK inflation printed below the Bank of England's 2.5% forecast, raising hopes for a June rate cut.",
  ...
  },
  {
    ...
    brief:
      "The world's largest asset manager completed its $12.5bn purchase of Global Infrastructure Partners, cementing its dominance in alternative asset management.",
...
const PLANS = [
  {
    ...
    description:
      "News explained simply, with context on why it matters for your studies, graduate applications, and finance interviews.",
    features: [
  ...
  },
  {
    ...
    description:
      "Technical depth, regulatory detail, and market data for lawyers, bankers, analysts, and fund managers.",
    features: [
...
function ArticleCard({ article, plan, detailLevel }) {
  ...
  return (
    ...
    >
      ...
      {/* Student callout — expanded detail level 1+ */}
      ...
      {article.verified && (
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
          <span style={{ fontSize: 11, color: C.green, fontFamily: "'DM Sans', sans-serif" }}>
            ✓ Verified
          </span>
      ...
      {/* Read more links — expanded */}
      {expanded && (
        <p style={{
          marginTop: 12, fontSize: 12, color: C.gold,
          fontFamily: "'DM Sans', sans-serif", fontWeight: 500,
        }}>
          Read full articles → {article.sources.join(" · ")}
        </p>
      )}
    </div>
  );
...
function OnboardingScreen({ onComplete }) {
      ...
      <div className="scale-in" style={{
        ...
        {[
          ...
          <div key={i} className="fade-up" style={{
            ...
            <p style={{
              fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: C.textMid, lineHeight: 1.5,
            }}>{item.text}</p>
          </div>
        ))}
  ...
  return (
    <div style={{
      minHeight: "100vh", background: C.bg, padding: "48px 20px 32px", overflowY: "auto",
    }}>
      <div className="fade-up" style={{ textAlign: "center", marginBottom: 32 }}>
        <h2 style={{
          fontFamily: "'DM Serif Display', serif",
          fontSize: 28, color: C.text, marginBottom: 8,
      ...
      <button
        ...
        style={{
          background: selectedPlan
            ? `linear-gradient(135deg, ${PLANS.find((p) => p.id === selectedPlan)?.accent}, ${
                selectedPlan === "pro" ? C.goldDim : "#3A5ECC"
              })`
            : C.border,
      ...
      >
        {selectedPlan
          ? `Continue as ${PLANS.find((p) => p.id === selectedPlan)?.label}`
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "0 20px" }}>
      ...
      <div style={{
        ...
        {/* Detail level toggle */}
        <div style={{
          display: "flex", background: C.surface, borderRadius: 8, padding: 3, gap: 2,
        }}>
          {["Headlines", "Brief", "Full"].map((label, i) => (
            <button key={label} onClick={() => setDetailLevel(i)} style={{
              flex: 1, padding: "7px 0", borderRadius: 6, border: "none",
              background: detailLevel === i
                ? plan === "student" ? C.student : C.gold
                : "transparent",
              color: detailLevel === i
                ? plan === "student" ? "#fff" : C.bg
                : C.textMid,
              fontFamily: "'DM Sans', sans-serif",
              fontSize: 12, fontWeight: detailLevel === i ? 600 : 400,
      ...
      <p style={{
        ...
        Phase 7 connects live AI-curated news here
      </p>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <p style={{
        ...
        Coming in a future phase. Check back soon.
      </p>
    </div>
  );
}
      ...
      {!onboarded ? (
        ...
        <>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {activeNav === "feed"    && (
              <FeedScreen plan={plan} detailLevel={detailLevel} setDetailLevel={setDetailLevel} />
            )}
            ...
            {NAV_ITEMS.map((item) => (
              ...
              >
                <span className="nav-icon" style={{
                  fontSize: 18, color: activeNav === item.id ? C.gold : C.textDim,
